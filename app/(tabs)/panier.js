...
export default function PanierScreen() {
            ...
            try {
              const res = await fetch(ENDPOINTS.commandes, {
                ...
                body: JSON.stringify({
                  ...
                  avec_livraison: livraison, // ← NOUVEAU
                }),
              });
              ...
              if (data.status === 'success') {
                ...
                Alert.alert(
                  '✅ Commande envoyée !',
                  `Votre commande a bien été enregistrée.${livraison ? '\n🛵 Livraison incluse.' : '\nVeuillez vous rendre sur les lieu du retrait.'}`
                );
              } else {
                Alert.alert('Erreur', data.message);
              }
            } catch (e) {
              Alert.alert('Problème de connexion', 'Impossible d\'envoyer votre commande.');
            } finally {
              setLoading(false);
            }
          },
  ...
  return (
    <>
      <StatusBar barStyle="light" backgroundColor="#FF6B35" />
            ...
            <View style={styles.recap}>
              ...
              <TouchableOpacity style={styles.livraisonRow} onPress={() => setLivraison(!livraison)}>
                ...
                <View style={styles.livraisonInfo}>
                  <Text style={styles.livraisonText}>Inclure la livraison</Text>
                  <Text style={styles.livraisonSub}>+{FRAIS_LIVRAISON} Fcfa · À domicile</Text>
                </View>
                <Ionicons name="bicycle-outline" size={20} color={livraison ? '#FF6B35' : '#ccc'} />
              </TouchableOpacity>

              {/* Badge mode sélectionné */}
              <View style={[styles.modeBadge, livraison ? styles.modeBadgeLivraison : styles.modeBadgeRetrait]}>
                <Ionicons
                  name={livraison ? 'bicycle-outline' : 'storefront-outline'}
                  size={14}
                  color={livraison ? '#9C27B0' : '#4CAF50'}
                />
                <Text style={[styles.modeBadgeText, { color: livraison ? '#9C27B0' : '#4CAF50' }]}>
                  {livraison ? 'Livraison à domicile' : 'Retrait sur place'}
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  livraisonRow: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 10, padding: 12, backgroundColor: '#f8f8f8', borderRadius: 12 },
  checkbox: {
    width: 22, height: 22, borderRadius: 6, borderWidth: 2,
    borderColor: '#ddd', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
  },
  checkboxActive: { backgroundColor: '#FF6B35', borderColor: '#FF6B35' },
  livraisonInfo: { flex: 1 },
  livraisonText: { fontSize: 14, color: '#1a1a1a', fontWeight: '600' },
  livraisonSub: { fontSize: 11, color: '#aaa', marginTop: 1 },

  modeBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, alignSelf: 'flex-start', marginBottom: 12 },
  modeBadgeLivraison: { backgroundColor: '#9C27B015' },
  modeBadgeRetrait: { backgroundColor: '#4CAF5015' },
